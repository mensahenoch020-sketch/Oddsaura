...
export default function SavedSlips() {
  ...
  return <section className="saved-section"><div><span>Saved predictions</span><h2>Your slips</h2></div>{loading ? <p>Loading your saved slips…</p> : error ? <p>{error}</p> : slips.length ? <div className="saved-grid">{slips.map((slip) => <article key={slip.id}><span>{new Date(slip.createdAt).toLocaleString()}</span><h3>{slip.name}</h3><p>{slip.picks.length} predicted {slip.picks.length === 1 ? "selection" : "selections"}</p><div><Link href={`/builder?slip=${encode(slip.picks)}`}>Open slip</Link><button type="button" onClick={() => void remove(slip.id)}>Remove</button></div></article>)}</div> : <div className="saved-empty"><p>You have not saved a prediction slip yet.</p><Link href="/builder">Choose predictions</Link></div>}</section>;
}
