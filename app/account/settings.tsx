...
export default function AccountSettings({ initialName, email }: { initialName: string; email: string }) {
  ...
  return <section className="settings-grid">
    ...
    <article className="session-card"><span>Session</span><h2>Signed-in devices</h2><p>Logging out removes the secure session from this device. Changing your password invalidates older sessions.</p><button type="button" onClick={() => void logout()} disabled={busy === "logout"}>{busy === "logout" ? "Logging out…" : "Log out"}</button></article>
  </section>;
}
