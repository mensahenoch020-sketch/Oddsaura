...
export default function ProductNavigation({ active, initialName = "", initialRole = "USER" }: { active: ProductArea; slipCount?: number; initialName?: string; initialRole?: "USER" | "ADMIN" }) {
  ...
  return <>
    ...
    <nav className="product-mobile-nav" aria-label="Mobile navigation">
      {items.map((item) => <Link key={item.id} href={item.href} aria-current={active === item.id ? "page" : undefined} className={active === item.id ? "active" : ""}>
        <span className="product-nav-icon"><NavIcon>{item.icon}</NavIcon></span>
