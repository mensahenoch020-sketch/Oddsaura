...
const items: Array<{ id: ProductArea; href: string; label: string; icon: React.ReactNode }> = [
  ...
  { id: "slip", href: "/builder#my-slip", label: "Slips", icon: <><path d="M5 4h14v16l-2.5-1.5L14 20l-2.5-1.5L9 20l-4-2.2z" /><path d="M8 8h8M8 12h8" /></> },
  ...
  return <>
    <header className="product-header">
      ...
      <nav className="product-desktop-nav" aria-label="Main navigation">
        {items.filter((item) => item.id !== "profile").map((item) => <Link key={item.id} href={item.href} aria-current={active === item.id ? "page" : undefined} className={active === item.id ? "active" : ""}>{item.id === "home" ? "New request" : "My slips"}{item.id === "slip" && slipCount > 0 ? <b>{slipCount}</b> : null}</Link>)}
