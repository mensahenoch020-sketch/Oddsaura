...
export default function ResultsPage() {
  ...
  const performance = useMemo(() => {
    const settled = periodTickets.filter((ticket) => ticket.status === "WON" || ticket.status === "LOST");
  ...
  return <main className="results-app">
    ...
    <nav className="results-statuses" aria-label="Ticket status">{([['SETTLED', 'Settled'], ['WON', 'Won'], ['LOST', 'Lost'], ['PENDING', 'Pending'], ['ALL', 'All']] as const).map(([value, label]) => <button type="button" key={value} className={statusFilter === value ? "active" : ""} onClick={() => { setStatusFilter(value); setVisibleTickets(20); }}>{label}</button>)}</nav>
