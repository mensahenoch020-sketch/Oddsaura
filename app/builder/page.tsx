    ...
    <section className="build-layout">
      <div className="build-board">
        ...
        <div className="build-fixtures">{visibleFixtureGroups.map((group) => <article id={`fixture-${encodeURIComponent(group.fixtureId)}`} key={group.fixtureId} className="build-fixture">
          ...
          <div className="build-markets build-predictions">{group.predictions.map((pick) => { const active = picks.some((item) => item.id === pick.id); const price = priceFor(pick); const history = pick.homeHistoryMatches != null && pick.awayHistoryMatches != null ? ` · history ${pick.homeHistoryMatches}/${pick.awayHistoryMatches}` : ""; return <button className={active ? "active" : ""} type="button" key={pick.id} onClick={() => choose(pick)}><span>{pick.market.name}</span><b>{pick.selection}</b><strong>{pick.quotedOdds == null && !liveOdds[pick.fixtureId] ? "EST. " : ""}{price?.toFixed(2) ?? pick.fairOdds.toFixed(2)}</strong><small>{liveOdds[pick.fixtureId] ? `${activeProvider.label} live` : pick.quotedOdds ? `Last quoted · verify${history}` : `Model estimate · verify${history}`}</small></button>; })}</div>
