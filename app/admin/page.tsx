...
import XReplyAssistant from "./x-reply-assistant";
...
export default function AdminPage() {
  ...
  const metricEntries = Object.entries(snapshot.metrics).filter(([key, value]) =>
  ...
  return <main className="adm-app">
    ...
    <section className="adm-content">
      ...
      <XReplyAssistant />
      <section className="adm-performance"><div className="adm-section-head"><h2>Historical prediction tests</h2><span>{performance.matches ? `${performance.matches} walk-forward matches` : "Backtest pending"}</span></div><p>{performance.methodology}</p><div><article><span>1X2 accuracy</span><strong>{performance.oneXTwoAccuracy == null ? "—" : `${Math.round(performance.oneXTwoAccuracy * 100)}%`}</strong></article><article><span>Ranked probability score</span><strong>{performance.rankedProbabilityScore == null ? "—" : performance.rankedProbabilityScore.toFixed(3)}</strong></article><article><span>Brier score</span><strong>{performance.brierScore == null ? "—" : performance.brierScore.toFixed(3)}</strong></article><article><span>Priced coverage</span><strong>{performance.coverage ? `${performance.coverage.pricedMatches}/${performance.coverage.evaluatedMatches}` : "—"}</strong></article></div></section>
      <section className="adm-performance"><div className="adm-section-head"><h2>Forward prediction proof</h2><span>Recorded before kickoff</span></div><p>This paper ledger records priced selections before matches start, then settles them from final scores. Confidence intervals show how uncertain the current result still is.</p><div><article><span>Recorded</span><strong>{snapshot.metrics.paperTrials?.recorded ?? 0}</strong></article><article><span>Settled</span><strong>{snapshot.metrics.paperTrials?.settled ?? 0}</strong></article><article><span>Won</span><strong>{snapshot.metrics.paperTrials?.won ?? 0}</strong></article><article><span>Hit rate</span><strong>{snapshot.metrics.paperTrials?.hitRate == null ? "—" : `${Math.round(snapshot.metrics.paperTrials.hitRate * 100)}%`}</strong><small>{snapshot.metrics.paperTrials?.hitRate95 ? `95%: ${(snapshot.metrics.paperTrials.hitRate95.low * 100).toFixed(1)}–${(snapshot.metrics.paperTrials.hitRate95.high * 100).toFixed(1)}%` : "Interval pending"}</small></article><article><span>Flat-stake ROI</span><strong>{snapshot.metrics.paperTrials?.flatStakeRoi == null ? "—" : `${(snapshot.metrics.paperTrials.flatStakeRoi * 100).toFixed(1)}%`}</strong><small>{snapshot.metrics.paperTrials?.flatStakeRoi95 ? `95%: ${(snapshot.metrics.paperTrials.flatStakeRoi95.low * 100).toFixed(1)}–${(snapshot.metrics.paperTrials.flatStakeRoi95.high * 100).toFixed(1)}%` : "Interval pending"}</small></article></div></section>
