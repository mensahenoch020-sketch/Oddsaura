    ...
    <section className={`converter-workspace${embedded ? " converter-workspace-embedded" : ""}`}>
      <form onSubmit={convert}>
        <div className="converter-route">
          <label><span>From</span><select value={source} onChange={(event) => { setSource(event.target.value as Provider); resetFeedback(); }}>{providers.map((item) => <option value={item.id} key={item.id}>{item.label}</option>)}</select><small>{sourceMeta.input}</small></label>
          <button type="button" className="converter-swap" onClick={swap} aria-label="Swap source and destination">⇄</button>
          <label><span>To</span><select value={destination} onChange={(event) => { setDestination(event.target.value as Provider); resetFeedback(); }}>{providers.map((item) => <option value={item.id} key={item.id}>{item.label}</option>)}</select><small>{destinationMeta.output}</small></label>
        </div>
        ...
        {destination === "bet9ja" && transferSelections.length ? <section className="converter-transfer"><header><div><span>Manual Bet9ja transfer</span><strong>All {transferSelections.length} selections preserved</strong><small>Bet9ja does not allow this page to fill another browser tab automatically. Copy the list and select each match on Bet9ja.</small></div><div><button type="button" onClick={() => void copyTransfer()}>{copied ? "Copied ✓" : "Copy complete slip"}</button><a href={destinationMeta.link} target="_blank" rel="noreferrer">Open Bet9ja ↗</a></div></header>{transferSelections.map((item, index) => <div key={`${item.fixtureId}-${index}`}><b>{index + 1}. {item.homeTeam} vs {item.awayTeam}</b><span>{item.marketName}: {item.selection}</span><small>{new Date(item.kickoff).toLocaleString()}</small></div>)}</section> : null}
      </form>
